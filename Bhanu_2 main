...
fprintf( " You are struck at an altitude on a snow mountain while skewing.\n" )
fprintf( " A drone can be flown over there which captures the topography of the region around you and converts it into a mathematical equation f(x,y).\n")
fprintf( " you have to reach the possible lowest altitude by following a steepest decent path at every step with the step size of %0.2f and find the lowest possible altitude w.r.t mean sea level that can be reached in %0.2f steps.\n",step_size,iterations)
fprintf( " function is defined as (x-%.2f)^2 + (%.2f-x*y)^2. Starting guess can be taken as [2,1]\n",a,b);
